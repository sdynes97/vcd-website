( function( $ ) {
	...
	$( 'form.tag-generator-panel .control-box :input' ).change( function() {
		...
		wp3cxc2c.taggen.normalize( $( this ) );
		wp3cxc2c.taggen.update( $form );
	} );

	$( 'input.insert-tag' ).click( function() {
		var $form = $( this ).closest( 'form.tag-generator-panel' );
		var tag = $form.find( 'input.tag' ).val();
		wp3cxc2c.taggen.insert( tag );
		tb_remove(); // close thickbox
		return false;
	} );

	wp3cxc2c.taggen.update = function( $form ) {
		...
		if ( $.isFunction( wp3cxc2c.taggen.update[ id ] ) ) {
			return wp3cxc2c.taggen.update[ id ].call( this, $form );
		}
		...
		$form.find( 'input.tag' ).each( function() {
			...
			var components = wp3cxc2c.taggen.compose( tag_type, $form );
			$( this ).val( components );
		} );
	...
	wp3cxc2c.taggen.update.captcha = function( $form ) {
		var captchac = wp3cxc2c.taggen.compose( 'captchac', $form );
		var captchar = wp3cxc2c.taggen.compose( 'captchar', $form );

		$form.find( 'input.tag' ).val( captchac + ' ' + captchar );
	};

	wp3cxc2c.taggen.compose = function( tagType, $form ) {
	...
	wp3cxc2c.taggen.normalize = function( $input ) {
		var val = $input.val();
		...
		if ( $input.is( ':checkbox.exclusive' ) ) {
			wp3cxc2c.taggen.exclusiveCheckbox( $input );
		}
	};

	wp3cxc2c.taggen.exclusiveCheckbox = function( $cb ) {
		if ( $cb.is( ':checked' ) ) {
			$cb.siblings( ':checkbox.exclusive' ).prop( 'checked', false );
		}
	};

	wp3cxc2c.taggen.insert = function( content ) {
		$( 'textarea#wp3cxc2c-form' ).each( function() {
			this.focus();
